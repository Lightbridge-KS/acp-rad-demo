---
name: compare
description: Compare the current study with the patient's priors — make the COMPARISON line true and state interval change on the organ lines that have a counterpart. Use when the radiologist asks to compare, or asks what has changed since a previous study.
metadata:
  hint: "[prior accession]"
---
Read `meta.json` for the current study's date. Read `/priors/index.md`: it lists every prior report of this patient with its accession, exam and date. **If the request names a prior accession, compare with that prior**; otherwise read every prior whose imaged anatomy overlaps the current study, whatever its modality (a chest radiograph or an abdominal CT's lung bases count for a chest study). Then read `sections/comparison.md` and `sections/findings.md`.
