...
- [ ] **S2. Agent loading over the namespace** (`agents/rad-agent`) — `adownload_files`; `EffectiveSkillsBackend`; `CompositeBackend` wiring; advertisement from the resolver; eager mention resolution; the four skills migrate to `<name>/SKILL.md`; **delete `skills.py`**; fix the inert `permissions=`. `pyyaml` becomes an explicit dependency.
...
- **Now:** S2 — the agent stops loading skills from its own package and starts resolving them over the namespace.
- **Next:** S3–S6 in order. Nothing here is parallelizable across steps — each leaves `just check` green and the demo runnable.
