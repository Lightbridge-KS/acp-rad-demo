...
- **Now:** S5 — the skill-authoring contract and a layer-3 `references/` example.
- **Next:** S6 (docs). `just smoke` has not been re-run since S2; the live gate needs port 8787 and a hosted model, and its assertions want extending to the layer switch. Nothing here is parallelizable across steps — each leaves `just check` green and the demo runnable.
