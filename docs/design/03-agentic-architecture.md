...
| Read-only zones | `FilesystemPermission(operations=["write"], paths=["/priors/**", "/templates/**", "/snippets/**", "/skills/**"], mode="deny")` — defense in depth; the editor refuses them too (`-32003`). **This was inert until 2026-08-31**: `create_deep_agent` passes `permissions=` into the `FilesystemMiddleware` it builds, and `agent.py` replaced that instance with one constructed without them, so only the editor's `assertWritable` stood. Fixed by passing `_permissions` through | authored config | ✅ |
| Skills (`/impression`, `/compare`, `/proofread`, `/qa`, `/stroke-protocol`) | Agent-Skills folders resolved across **builtin → house → personal** (ADR 0004); `skills.py` composes them and `SkillsMiddleware(sources=["/skills/effective/"])` advertises them to the model. `available_commands_update` is built from the *resolved* set, so the menu reflects the institution's and the radiologist's layers | ours + authored content | ✅ |
| Skill composition | `EffectiveSkillsBackend` synthesizes `/skills/effective/{name}/SKILL.md`. deepagents' `SkillMetadata` carries frontmatter only — never the body — so composition must resolve behind one readable path | ours | ✅ |
| Read-only zones, continued | `/skills/**` joins them: an agent that could rewrite its own skills could rewrite the checks it is about to be judged by | authored config | ✅ |
...
- ~~💡 **`skills=` transport**~~ — **decided 2026-08-31 (ADR 0004)**, and not as sketched. There is one route, `CompositeBackend(default=AcpClientBackend, routes={"/skills/effective/": EffectiveSkillsBackend})`, and it points at a *synthesized* layer rather than a local `FilesystemBackend`. The builtin layer is deliberately **not** exposed as a path: a model that read `/skills/builtin/qa/SKILL.md` would get the un-composed base and silently miss the house and personal checks — the failure sealing exists to prevent.
