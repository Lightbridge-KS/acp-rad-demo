...
>
> 1. Wording. Fix spelling, grammar, capitalisation after a label, units and number style (`9-mm nodule`, `2.5-mm slice thickness`), house grammar (bold label lines, `- ` impression items, no headings), and duplicated or dangling phrases. Never change a finding's meaning, size, certainty, or side. The HISTORY is what the referrer told you: fix only its spelling and grammar, never its wording, certainty, or diagnosis.
