...
| `agent/` | `client()` wiring: handlers for `session/update`, `request_permission`, `fs/*`, `_rad/critical_finding`; `session/new` with `_meta.rad`; cancel. |
