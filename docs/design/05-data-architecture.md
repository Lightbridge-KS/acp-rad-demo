...
| Report status · `shortPrelim` | React state in `App.tsx` (`status`, `shortPrelim`) | `statusRef` read by `ReportStore.reportStatus()` | editor commands (`/short-prelim`, fold-in); status transitions are slice 6 | `assertWritable`, header pill, command context | Not persisted; a reload resets to the fixture's value. |
...
- **Write lock.** `reportStatus === "final"` ⇒ every `fs/write_text_file` is refused with `-32003` in two places (`store.assertWritable`, `connection.ts`), audited `fs.write.refused`. Transitions (Prelim, Sign off, the QA gate) are slice 6.
...
- **Persistence (slice 6).** Where the worklist rests and whether a report buffer survives a reload or a session change (`session/load` is out of v0.1). Today a reload discards the report.
