/**
 * One case, open: the report (Quill), its stores, the agent session and the sidebar.
 * Mounted per case by the shell (`App.tsx`) with `key={fixture.id}`, so every piece of
 * per-case state — status, short prelim, proposals, flags, audit, connection — starts
 * fresh on a worklist switch. The shell injects its controls (`headerStart`, `banner`).
 *
 * Owns the report lifecycle (design 02 §5.2) and the QA gate (04 §3.5): the pill offers
 * the transitions the role may take; a click runs `qaGate.ts` and executes its effects.
 */
...
import { deterministicGate, describeBlockers, transitionLabel, transitionsFor, type BlockerKind, type Role, type Transition } from "./report/lifecycle.ts";
import { ReportEditor } from "./report/ReportEditor.tsx";
import { AI_UNREVIEWED, clearAllUnreviewed, clearFlagOps, decideHunkOps, discardHunksOps, flagLineIndex, flagLineOps, hunkLineIndex, isInsertLine, lineIndex, lineLength, lineText, overlayOps, pendingHunkIds, splitLines, unreviewedLineCount } from "./report/overlay.ts";
...
import { GATE_IDLE, GATE_TIMEOUT_MS, gateStep, type GateAction, type GateState, type SkipReason } from "./report/qaGate.ts";
...
export type WorkspaceHandle = {
  dirty: () => Dirty;
  /** Stop a running turn (best effort — unmount closes the socket anyway). */
  cancel: () => Promise<void>;
};
...
type Props = {
  ...
  /** Shell panel rendered under the header (the worklist's inline confirm). */
  banner?: ReactNode;
};

const SKIP_TEXT: Record<SkipReason, string> = {
  short_prelim: "short prelim",
  agent_absent: "the agent is not connected",
  level: "the agent does not raise flags (Level < 2)",
  timeout: "the agent did not answer in time",
  cancelled: "the check was stopped",
...
export function Workspace({ fixture, role, ref, headerStart, banner }: Props) {
  ...
  // QA gate; the short-prelim property is set by /short-prelim and cleared by the fold-in.
  const [status, setStatus] = useState<ReportStatus>(fixture.session.reportStatus);
  const [shortPrelim, setShortPrelim] = useState<boolean>(fixture.session.shortPrelim);
  const statusRef = useRef(status);
  statusRef.current = status;
  const [gate, setGate] = useState<GateState>(GATE_IDLE);
  ...
  const scrollTo = useCallback(
    (idx: number) => {
      if (!quill || idx < 0) return;
      quill.setSelection(idx, 0, "silent");
      quill.scrollSelectionIntoView();
    },
    [quill],
  );
  const locateFlag = useCallback((id: string) => quill && scrollTo(flagLineIndex(currentOps(quill), id)), [quill, scrollTo]);
  ...
  // ---- QA gate (design 04 §3.5): the reducer decides, this executes its effects ----------

  const runGate = useCallback(
    (action: GateAction) => {
      const step = gateStep(gateRef.current, action);
      gateRef.current = step.state;
      setGate(step.state);
      if (step.state.phase !== "running" && gateTimer.current) {
      ...
      for (const effect of step.effects) {
        switch (effect.type) {
          case "prompt": {
            // The agent never knows it is a gate: it receives the literal skill.
            if (!agentPort) break;
            audit.record("command.qa", { outcome: "gate" });
            dispatch({ type: "user", text: effect.text });
            gateTimer.current = setTimeout(() => runGate({ type: "timeout" }), GATE_TIMEOUT_MS);
            void agentPort.prompt(effect.text).then((stopReason) => runGate({ type: "turn_end", stopReason, flags: flags.list() }));
            break;
          }
          case "cancel":
            void agentPort?.cancel();
            break;
          case "transition":
            setStatus(effect.status);
            audit.record("status.changed", { outcome: effect.status });
            break;
          case "audit":
            audit.record(effect.event, { ...(effect.outcome ? { outcome: effect.outcome } : {}), ...(effect.flagIds ? { flagIds: effect.flagIds } : {}) });
            break;
        }
      }
    },
    [agentPort, audit, flags],
  );
  useEffect(
    () => () => {
  ...
  const startGate = useCallback(
    (transition: Transition) => {
      if (!quill || !store || state.isRunning) return;
      const ops = currentOps(quill);
      runGate({
        type: "start",
        transition,
        gate: deterministicGate({ pending: pendingHunkIds(ops).length, unreviewed: unreviewedLineCount(ops), markdown: store.reportMarkdown() }),
        shortPrelim,
        agentLevel: header.status === "ready" ? header.level : undefined,
        flagCount: flags.list().length,
      });
    },
    [quill, store, state.isRunning, runGate, shortPrelim, header.status, header.level, flags],
  );

  /** Scroll to the first thing a blocker points at. */
  const showBlocker = useCallback(
    (kind: BlockerKind) => {
      if (!quill) return;
      const ops = currentOps(quill);
      if (kind === "pending") {
        const id = pendingHunkIds(ops)[0];
        if (id) scrollTo(hunkLineIndex(ops, id));
        return;
      }
      const lines = splitLines(ops);
      const i = lines.findIndex((l) =>
        kind === "unreviewed" ? l.runs.some((r) => (r.attributes as Record<string, unknown> | undefined)?.[AI_UNREVIEWED]) : kind === "blank" ? /_{2,}/.test(lineText(l)) : false,
      );
      if (i >= 0) scrollTo(lineIndex(lines, i));
    },
    [quill, scrollTo],
  );
  ...
  useImperativeHandle(
    ...
    () => ({
      ...
      cancel: async () => {
        if (state.isRunning) await agentPort?.cancel();
      },
    }),
    [proposals, quill, state.isRunning, flags, agentPort],
  );
  ...
  const runCommand = useCallback(
    (command: Command, arg?: string) => {
      ...
      if (command.kind === "skill") {
        ...
        if (state.isRunning) {
          setHint("the agent is busy — wait for the turn to end or press Stop");
          return;
        }
  ...
  const transitions = transitionsFor(status, role);
  const gateBusy = gate.phase !== "idle" || state.isRunning;

  return (
    <div className="grid h-full grid-cols-[minmax(0,1fr)_400px] grid-rows-[auto_auto_minmax(0,1fr)]">
      <header className="col-span-2 flex items-center gap-3 border-b border-gray-200 px-4 py-2 text-sm">
        ...
        <span className="ml-auto flex items-center gap-2 text-xs">
          ...
          <span className={`flex items-center gap-2 rounded px-2 py-0.5 ${status === "final" ? "bg-gray-800 text-white" : "bg-amber-100 text-amber-800"}`}>
            <span data-testid="status">
              {status}
              {shortPrelim ? " · short prelim" : ""}
            </span>
            {transitions.map((t) => (
              <button key={t} type="button" data-testid={`transition-${t}`} disabled={gateBusy} className="rounded bg-white/80 px-2 py-0.5 font-medium text-gray-900 shadow-sm disabled:opacity-40" onClick={() => startGate(t)}>
                {transitionLabel(t)}
              </button>
            ))}
          </span>
      ...
      <div className="col-span-2">
        {banner}
        {gate.phase !== "idle" && (
          <div data-testid="gate" data-phase={gate.phase} className="flex items-center gap-3 border-b border-gray-200 bg-gray-50 px-4 py-2 text-sm">
            {gate.phase === "refused" && (
              <>
                <span>
                  Cannot {transitionLabel(gate.transition).toLowerCase()} yet: <span className="font-medium">{describeBlockers(gate.blockers)}</span>
                </span>
                {gate.blockers.map((b) => (
                  <button key={b.kind} type="button" className="text-xs underline" onClick={() => showBlocker(b.kind)}>
                    show {b.kind === "empty" ? "report" : b.kind}
                  </button>
                ))}
                <button type="button" className="ml-auto rounded border border-gray-300 px-2 py-0.5 text-xs" onClick={() => runGate({ type: "dismiss" })}>
                  Dismiss
                </button>
              </>
            )}
            {gate.phase === "running" && <span className="animate-pulse text-gray-600">Checking the report with the agent (/qa)…</span>}
            {gate.phase === "flagged" && (
              <>
                <span>
                  The agent raised <span className="font-medium">{gate.flagIds.length} flag{gate.flagIds.length === 1 ? "" : "s"}</span> — review them in the sidebar before you {transitionLabel(gate.transition).toLowerCase()}.
                </span>
                <button type="button" data-testid="gate-review" className="ml-auto rounded bg-sky-600 px-2 py-0.5 text-xs text-white" onClick={() => runGate({ type: "review" })}>
                  Review
                </button>
                <button type="button" data-testid="gate-override" className="rounded border border-gray-400 px-2 py-0.5 text-xs" onClick={() => runGate({ type: "override" })}>
                  {transitionLabel(gate.transition)} anyway
                </button>
              </>
            )}
            {gate.phase === "unavailable" && (
              <>
                <span>
                  QA unavailable — {SKIP_TEXT[gate.reason]}.
                </span>
                <button type="button" data-testid="gate-override" className="ml-auto rounded border border-gray-400 px-2 py-0.5 text-xs" onClick={() => runGate({ type: "override" })}>
                  {transitionLabel(gate.transition)} anyway
                </button>
                <button type="button" className="rounded border border-gray-300 px-2 py-0.5 text-xs" onClick={() => runGate({ type: "dismiss" })}>
                  Dismiss
                </button>
              </>
            )}
